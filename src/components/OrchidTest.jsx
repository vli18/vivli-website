  ...
  useEffect(() => {
    ...
    const orchidFragment = `
      ...
      void main(){
        ...
        for (int i = 0; i < ${ORCHID_COUNT}; i++){
          ...
          float t = (u_time + offs) * 0.7;
          float bloom = 0.4 + 0.3 * sin(t * 2.0);
          float pulse = 0.6 + 0.4 * sin(t * 1.3);
          p *= (1.0 + bloom);
    ...
    const asciiFragment = `
      ...
      void main() {
        ...
        vec2 cc = floor(frag / charSize) * charSize;
        vec2 cuv = cc / resolution;

        vec4 color = texture2D(tDiffuse, cuv);
        float b = (color.r + color.g + color.b) / 3.0;

        vec2 cp = mod(frag, charSize) / charSize;
        float ch = 0.0;
    ...
    const init = () => {
      ...
      rt = new THREE.WebGLRenderTarget(width * dpr, height * dpr);
